class Arrow {
    ...
    checkBall(ball){
      ...
      {
        this.angle = Math.atan((ball.velocity.dy) / (ball.velocity.dx)) * ((15 + Math.PI) * Math.PI);
      }
      else
      {
        this.angle = Math.atan((ball.velocity.dy) / (ball.velocity.dx)) * ((15 + Math.PI) * Math.PI) + 180;
      }
    }
}
